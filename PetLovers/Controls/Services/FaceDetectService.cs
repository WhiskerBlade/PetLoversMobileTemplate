...
using NcnnDotNet;
using PetLovers.Controls.Interfaces;
using PetLovers.Models;

namespace PetLovers.Controls.Services
{
    public sealed class FaceDetectService : IDetectService
    {
        ...
        {
            var resourcePrefix = "PetLovers.FaceData.";
            // note that the prefix includes the trailing period '.' that is required
            var files = new[] { "RFB-320.bin", "RFB-320.param" };
            var assembly = System.Reflection.IntrospectionExtensions.GetTypeInfo(typeof(FaceDetectService)).Assembly;
